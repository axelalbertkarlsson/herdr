...
- Pane cursors now ignore positions taken from inside an application's own unfinished synchronized redraw, and wait for a real cursor placement when such a redraw ends on painted text, so holding a key in an agent's composer no longer flashes the cursor onto its status line.
